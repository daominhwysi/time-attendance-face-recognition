...
from lib.uniface.onnx_utils import create_onnx_session
...
class BaseRecognizer(ABC):
    ...
    def _initialize_model(self) -> None:
        ...
        try:
            ...
            if model_input_size != self.input_size:
                print(f"Model input size {model_input_size} differs from configured size {self.input_size}")
            ...
            print(f"Successfully initialized face encoder from {self.model_path}")

        except Exception as e:
            print(f"Failed to load face encoder model from '{self.model_path}'")
    ...
    def get_embedding(self, image: np.ndarray, landmarks: np.ndarray = None, use_landmarks: bool = True) -> np.ndarray:
        """
        Extracts face embedding from an image.

        Args:
            image: Input face image (BGR format).
            landmarks: Facial landmarks (optional, 5 points for alignment).
            use_landmarks: Whether to align face using landmarks before embedding.

        Returns:
            Face embedding vector (typically 512-dimensional).
        """
        # Align face only if user chooses and landmarks exist
        if use_landmarks and landmarks is not None:
    ...
    def get_normalized_embedding(self, image: np.ndarray, landmarks: np.ndarray = None, use_landmarks: bool = True) -> np.ndarray:
        """
        Extracts an L2-normalized face embedding vector from an image.
        ...
        """
        embedding = self.get_embedding(image, landmarks, use_landmarks)
